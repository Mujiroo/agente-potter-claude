#!/usr/bin/env python3
"""Generador en ESPAÑOL de «Pasatiempos Tranquilos» (sopas de letras + sudokus + laberintos).

Copia de calm-cozy/tools/generar_mixto.py con textos en español, abecedario con Ñ
y filtro de groserías en español además del inglés.
...
def main(src, dst):
    ...
    for kind, idx in seq:
        if kind == "w":
            ...
            page(f'<div class="tag">Sudoku {si}</div><h1>Fácil</h1>{PZ.sudoku_svg(pz)}'
                 '<div class="hint">Del 1 al 9, sin repetir en ninguna fila, columna ni cuadro.</div>', "center")
        else:
            mi += 1
            mz = maze_pool[mi - 1]
    ...
    for k in range(0, len(solved_s), 6):  # seis sudokus por página (2 x 3)
        ...
        page(f'<div class="tag">Soluciones</div><h1>Sudoku</h1><div class="sol6">{items}</div>')
    for k in range(0, len(solved_m), 4):  # cuatro laberintos por página (2 x 2)
        items = "".join(f'<div class="item">{PZ.maze_svg(mz, solve=True, cell=26, lw=6)}<div>Laberinto {i}</div></div>'
                        for i, mz in solved_m[k:k + 4])
        page(f'<div class="tag">Soluciones</div><h1>Laberintos</h1><div class="sol4">{items}</div>')
